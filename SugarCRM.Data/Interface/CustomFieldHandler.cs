...
namespace SugarCRM.Data.Interface
{
    public class CustomFieldHandler : Integration.Abstract.CustomFieldHandler
    {
        ...
        {
            if(inputObject is AbstractSugarCRMDataWithCustomFields)
            {
                var customFields = ((AbstractSugarCRMDataWithCustomFields)inputObject).CustomFields;
        ...
        public override bool SetValueCustomField(object inputObject, string propertyName, object propertyVal)
        {
            if (inputObject is AbstractSugarCRMDataWithCustomFields)
            {
                ...
                if (customFields == null)
                {
                    ((AbstractSugarCRMDataWithCustomFields)inputObject).CustomFields = new List<Field>();
                    customFields = ((AbstractSugarCRMDataWithCustomFields)inputObject).CustomFields;
                }
                var customFieldMatch = customFields.Find(x => x.Key == propertyName);
                if (customFieldMatch == null)
                {
                    var customFieldNew = new SugarCRM.DataModels.Field();
